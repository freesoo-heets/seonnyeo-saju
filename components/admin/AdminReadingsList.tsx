...
export default function AdminReadingsList({ readings }: { readings: Reading[] }) {
  ...
  const counts = useMemo(() => {
    ...
    for (const reading of readings) result[reading.status] = (result[reading.status] ?? 0) + 1;
    return result;
  }, [readings]);
  ...
  const filteredReadings = useMemo(() => {
    ...
    return readings
      .filter((reading) => {
        ...
        if (connectionFilter === "linked" && !reading.requester_nickname) return false;
        if (connectionFilter === "unlinked" && reading.requester_nickname) return false;
  ...
  }, [readings, keyword, statusFilter, connectionFilter, sortOrder]);
  ...
  const hasFilter = keyword || statusFilter !== "all" || connectionFilter !== "all" || sortOrder !== "newest";
    ...
    <div className="mt-5">
      <div className="flex gap-2 overflow-x-auto pb-1">
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              {tab.text}
              <span className={`ml-2 text-xs ${active ? "text-white/70" : "text-[#aaa09a]"}`}>{counts[tab.value] ?? 0}</span>
            </button>
          );
        })}
      </div>

      <div className="mt-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_150px_150px_auto]">
        <div className="relative min-w-0">
          <input
            value={keyword}
            onChange={(e) => setKeyword(e.target.value)}
            placeholder="닉네임, 사주 대상, 상담번호, 질문 검색"
        ...
        <select
          ...
          className="w-full rounded-2xl border border-[#dfd3c5] bg-[#fffdf9] px-3 py-3 text-sm font-semibold text-[#64585c] outline-none"
        >
          <option value="all">연결 전체</option>
          <option value="linked">회원 연결</option>
          <option value="unlinked">미연결만</option>
        </select>
        ...
        <button
          ...
          disabled={!hasFilter}
          className="rounded-2xl border border-[#dfd3c5] bg-[#faf6ef] px-4 py-3 text-sm font-bold text-[#74676d] disabled:cursor-default disabled:opacity-40"
        >
          초기화
        </button>
      </div>

      <div className="mt-3 flex items-center justify-between gap-3 text-xs text-[#9a8e88]">
        <span>{connectionFilter === "unlinked" ? "미연결 상담만 표시" : connectionFilter === "linked" ? "회원 연결 상담만 표시" : "전체 연결 상태"}</span>
        <span>검색 결과 <strong className="text-[#4e3d49]">{filteredReadings.length}</strong>건</span>
      </div>
      ...
      {filteredReadings.length === 0 ? (
        <div className="mt-4 rounded-[24px] border border-dashed border-[#d8c9b8] bg-[#fbf7f1] px-5 py-14 text-center">
          ...
          <p className="mt-2 text-sm text-[#968a84]">검색어나 필터 조건을 변경해보세요.</p>
          {hasFilter ? (
            <button type="button" onClick={resetFilters} className="mt-5 rounded-xl border border-[#cfbda8] bg-white px-4 py-2 text-sm font-bold text-[#684f60]">
              전체 상담 보기
            </button>
          ) : null}
        </div>
        ...
        <>
          <div className="mt-4 hidden overflow-hidden rounded-[22px] border border-[#e5dacd] lg:block">
                  ...
                  return (
                    <tr key={reading.id} className="border-t border-[#eee5db] transition hover:bg-[#fcf8f3]">
                      ...
                      <td className="px-4 py-4">
                        <div className="font-bold text-[#4b3748]">{reading.requester_nickname ?? "미연결"}</div>
                        {!reading.requester_nickname ? <div className="mt-1 text-[10px] text-[#b09f95]">기존 비회원 접수</div> : null}
                      </td>
                      <td className="px-4 py-4">
                      ...
                      <td className="px-4 py-4">
                        <span className={`inline-flex rounded-full border px-3 py-1 text-xs font-bold ${status.badge}`}>{status.text}</span>
                      </td>
                      <td className="px-4 py-4 text-sm text-[#817570]">{formatDate(reading.created_at)}</td>
                      <td className="px-4 py-4 text-right">
                        <Link href={`/admin/readings/${reading.id}`} className="inline-flex rounded-xl border border-[#cdbb9e] bg-[#fffaf2] px-4 py-2 text-xs font-bold text-[#62445b] transition hover:bg-[#62445b] hover:text-white">
                          상세 보기
                        </Link>
                      </td>
                    </tr>
            ...
            {filteredReadings.map((reading) => {
              ...
              return (
                <Link
                  ...
                  className="block rounded-[22px] border border-[#e5dacd] bg-[#fffdf9] p-4 shadow-[0_6px_20px_rgba(68,45,58,0.04)] transition active:scale-[0.99]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-[11px] font-bold text-[#a38965]">{reading.consultation_number}</div>
                      <div className="mt-2 flex flex-wrap items-baseline gap-2">
                        <strong className="truncate text-base text-[#4b3748]">{reading.requester_nickname ?? "미연결"}</strong>
                        <span className="text-[11px] text-[#9b908b]">신청자</span>
                      </div>
                    </div>
                    <span className={`shrink-0 rounded-full border px-3 py-1 text-[11px] font-bold ${status.badge}`}>{status.text}</span>
                  </div>
                  <div className="mt-3 rounded-xl bg-[#faf6f0] px-3 py-2.5">
                    <div className="text-xs text-[#9b908b]">사주 대상</div>
                    <div className="mt-1 truncate text-sm font-semibold text-[#62555b]">{reading.customer_name || "-"}</div>
                  </div>
                  <div className="mt-3 flex items-center justify-between text-xs text-[#9b908b]">
